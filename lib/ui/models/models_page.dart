...
class ModelsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(S.of(context).models),
        actions: [
          IconButton(
            icon: const Icon(Icons.search),
            tooltip: S.of(context).searchModels,
            onPressed: () {
              Navigator.push(
                context,
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text(S.of(context).noModels,
                      style: Theme.of(context)
                          .textTheme
                          .titleMedium
                          ?.copyWith(color: colorScheme.outline)),
                  const SizedBox(height: 8),
                  Text(S.of(context).tapSearchToDownload,
                      style: Theme.of(context)
                          .textTheme
                          .bodyMedium
                          ?.copyWith(
                  ...
                  FilledButton.icon(
                    ...
                    icon: const Icon(Icons.search),
                    label: Text(S.of(context).searchModels),
                  ),
                ],
              ),
            );
          ...
          return ListView(
            ...
            children: [
              if (downloading.isNotEmpty) ...[
                _SectionHeader(title: S.of(context).downloading, count: downloading.length),
                ...downloading.map((m) => LocalModelTile(model: m)),
                const SizedBox(height: 16),
              ],
              if (completed.isNotEmpty) ...[
                _SectionHeader(title: S.of(context).completed, count: completed.length),
                ...completed.map((m) => LocalModelTile(model: m)),
                const SizedBox(height: 16),
              ],
              if (failed.isNotEmpty) ...[
                _SectionHeader(title: S.of(context).failed, count: failed.length),
